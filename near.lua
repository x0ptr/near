...
local spellIDCleanser = CreateFrame("Frame"):CreateFontString(nil, "OVERLAY")
spellIDCleanser:SetFont(GameFontNormal:GetFont())
spellIDCleanser:Hide()
        ...
        if not enemyNameplatesEnabled then
            ...
            for i = 1, 10 do
                ...
                if enemies[i] then
                    ...
                    frame.healthText:SetText(string.format("%3d%%", entity.healthPercent))

                    -- Set health background bar width and color (full frame width)
                    local frameWidth = frame:GetWidth()
                    frame.healthBg:SetWidth(frameWidth - 10)
                    ...
                    frame:RegisterUnitEvent("UNIT_SPELLCAST_START", entity.unit)
                    frame:RegisterUnitEvent("UNIT_SPELLCAST_STOP", entity.unit)
                    frame:RegisterUnitEvent("UNIT_SPELLCAST_CHANNEL_START", entity.unit)
                    frame:RegisterUnitEvent("UNIT_SPELLCAST_CHANNEL_STOP", entity.unit)

                    -- In your event handler, track the state
                    frame:SetScript("OnEvent", function(self, event, unit)
                        ...
                        if nameplate and nameplate.UnitFrame and nameplate.UnitFrame.castBar then
                            ...
                            shield = castBar.BorderShield
                        end
                        local name, text, texture, startTime, endTime, isTradeSkill, castID, notInterruptible, spellID = UnitCastingInfo(unit);

                        if name then
                            if shield and shield:IsShown() then
                                frame.castText:SetText("|cFF666666> " .. name .. "|r")
                            else
                                local now = GetTime()
                                if now - lastSoundTime >= 3 then
                                    PlaySound(8959)  -- Ready check sound, or use your own
                                    lastSoundTime = now
                                end
                                frame.castText:SetText("|cFFFF8800>>> KICK " .. name .. " <<<|r")
                            end
                        else
                            frame.castText:SetText("")
                            return
